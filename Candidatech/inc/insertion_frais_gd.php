...
$query = "SELECT NOF_NO FROM NOTE_FRAIS WHERE NOF_ANNEE = " . $_POST['annee'] . " AND NOF_MOIS = " . $_POST['mois'] . " AND COL_NO = " . $collab . " AND TYF_NO = 3";
...
$query = "INSERT INTO NOTE_FRAIS (NOF_ANNEE, NOF_MOIS, COL_NO, TYF_NO, NOF_NSEQUENTIEL) VALUES (" . $_POST['annee'] . "," . $_POST['mois'] . "," . $collab . ", 3, '" . $nsequentiel . "')";
$result = $connexion->query ($query);
$LigneMail[0] = '<p>Une demande de frais grand déplacement a été créée par ' . $_SESSION['nom'] . ' ' . $_SESSION['prenom'] . '</b> pour <b>' . nomMois($_POST['mois']) . ' ' . $_POST['annee'] . '</b><br/><br/>Détails :</p>';

//creation de chaque ligne avec objet et le montant a l'unité
$id = mysqli_insert_id ($connexion);
for ($i = 1; $i <= ($_POST['nbligne']); $i++)
{
    $query = "INSERT INTO LIGNE_FRAIS (LIF_OBJET, LIF_JOUR, LIF_NBJ_W, LIF_DETAIL, LIF_MONTANT, NOF_NO) VALUES ('" . $_POST['objet' . $i] . "', '" . $_POST['jour' . $i] . "', '" . $_POST['nbjW' . $i] . "', '" . $_POST['detail' . $i] . "', '" . $_POST['total' . $i] . "', " . $id . ")";
    $result = $connexion->query ($query);
    $LigneMail[$i] = '<p>le ' . $_POST['jour' . $i] . ' : ' . $_POST['objet' . $i] . ' ' . $_POST['nbjW' . $i] . 'jrs : ' . $_POST['total' . $i] . '&euro;<br/>';
    $LigneMail[$i] .= 'Détail : ' . $_POST['detail' . $i] . '</p>';
}
...
<html>
    <div align='center' class=''>
        <font color='green'>Note de frais de grand déplacement enregistrée</font>
    </div>
</html>