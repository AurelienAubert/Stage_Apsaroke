...
final class SFTP {
	...
	public function ls($directory = null) {
		$list = array();
                ...
                //$list = ftp_rawlist($this->_stream, $directory, false);
	...
	/**
	 * Get list of description of files/directories in directory, may be recursive
	 *
	 * @param string $directory
	 * @param bool $recursive
	 * @return array
	 */
	public function lsdet($directory = null, $recursive = false) {
		$list = array();
                 
		// attempt to get list
		if($list = ftp_rawlist($this->_stream, $directory, false)) {
                    $items = array(); 

                    foreach ($list as $child) { 
                        $chunks = preg_split("/\s+/", $child); 
                        list($item['rights'], $item['number'], $item['user'], $item['group'], $item['size'], $item['month'], $item['day'], $item['time'], $item['file']) = $chunks; 
